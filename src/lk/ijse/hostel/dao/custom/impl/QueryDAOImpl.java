...
public class QueryDAOImpl implements QueryDAO {
    ...
    @Override
    public List<CustomEntity> getRoomReserveList() {
        Session session = FactoryConfiguration.getInstance().getSession();
        Transaction transaction = session.beginTransaction();

        String hql="SELECT r.id,r.room.id,r.student.id,r.student.name.f_name,r.student.address,r.student.contact.phone_Num,r.endDate,r.room.key_money,r.lessAmount ,r.status FROM Reservation r INNER JOIN Student s ON r.student=s.student_id ";
        Query query = session.createQuery(hql);
        List<Object[]> list = query.list();
        List<CustomEntity> customEntityList =new ArrayList<>();
        for (Object[] ob1:list) {
